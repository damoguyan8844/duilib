...
namespace DuiLib
{
	...
	void CContainerUI::LineUp()
	{
		...
		if(m_pVerticalScrollBar) cyLine = m_pVerticalScrollBar->GetLineSize();
	...
	void CContainerUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcsicmp(pstrName, _T("mousechild")) == 0 ) SetMouseChildEnabled(_tcscmp(pstrValue, _T("true")) == 0);
